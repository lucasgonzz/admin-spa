<template>
  <base-modal
    ...
    <div class="image-annotation-editor">
      ...
      <div class="image-annotation-toolbar d-flex flex-wrap align-items-center gap-2 mb-2">
        <div class="btn-group btn-group-sm" role="group" aria-label="Herramientas de dibujo">
          <button
            type="button"
        ...
        <div class="btn-group btn-group-sm" role="group" aria-label="Color del trazo">
          <button
    ...
    <template #footer>
      <button type="button" class="btn btn-secondary" @click="on_cancel">Cancelar</button>
      <button
        type="button"
...
export default {
  ...
  data() {
    return {
      ...
      object_url: null,
      /** Dimensiones lógicas del canvas (px). */
      canvas_width: 0,
      canvas_height: 0,
  ...
  watch: {
    /**
     * Al abrir con un archivo nuevo, carga la imagen en el canvas.
     *
     * @param {boolean} visible
     */
    show(visible) {
      if (visible && this.source_file) {
  ...
  methods: {
    /**
     * Sincroniza v-model del modal padre.
     *
     * @param {boolean} value
     */
    on_modal_show_update(value) {
    ...
    /**
     * Cierra sin confirmar y notifica al padre.
     */
    on_cancel() {
    ...
    finish_stroke() {
      ...
      this.is_drawing = false
      /* Ignorar trazos vacíos (clic sin arrastre en rect/arrow). */
