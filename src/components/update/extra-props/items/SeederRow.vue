<template>
  <tbody>
    <tr>
      ...
      <td>
        <small class="text-muted">{{ item.version_seeder ? item.version_seeder.description : '—' }}</small>
        <!-- Badge que indica si el seeder se ejecuta una vez por BD o una vez por usuario/tenant -->
        <span
          v-if="item.version_seeder && item.version_seeder.run_scope"
          class="badge ms-1"
...
<script>
import { resolve_update_seeder_command_text } from '@/utils/deployment_run_command_resolver'

/**
 * Fila de un UpdateSeeder: muestra comando a ejecutar, permite copiar al portapapeles
 * y marcar como exitoso automáticamente, o marcar como fallido/pendiente.
 */
export default {
  name: 'SeederRow',
  props: {
  ...
  computed: {
    input_id() {
      return 'upd-seeder-' + this.item.id
    },
    seeder_command_text() {
      return resolve_update_seeder_command_text(this.item, this.client)
    },
    seeder_class() {
      return this.item.version_seeder ? this.item.version_seeder.seeder_class : ''
    },
    version_label() {
      return this.item.version_seeder && this.item.version_seeder.version
        ? this.item.version_seeder.version.version
        : '—'
    },
    status_badge_class() {
     ...
     * per_database → gris neutro; per_user → azul (requiere atención por tenant).
     * @returns {string}
     */
    run_scope_badge_class() {
      const scope = this.item.version_seeder && this.item.version_seeder.run_scope
      if (scope === 'per_user') return 'text-bg-info'
